...
import { Edit, Eye, EyeOff, Footprints, PlusCircle, Save, Trash2, X } from 'lucide-react';
...
export default function ShoeSizeMaster({ data }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/shoe-sizes/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
            ...
            post('/settings/masterTables/shoe-sizes', {
                onSuccess: () => {
                    reset();
    ...
    const handleEdit = (item: ShoeSize) => {
        setEditingId(item.id);
        setData({
            description_shoesize: item.description_shoesize,
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            title: '¿Estás seguro?',
            text: 'La talla se eliminará permanentemente.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/shoe-sizes/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: ShoeSize) => (
        <Card
            key={item.id}
            ...
            <CardHeader className="pb-2">
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Descripción Talla Zapato:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.description_shoesize}</span>
                    </div>
                </div>
            </CardHeader>
    ...
    const renderRow = (item: ShoeSize) => (
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-stone-100 dark:bg-stone-800">
                    <Footprints className="h-4 w-4 text-stone-600 dark:text-stone-400" />
                </div>

                <div className="flex flex-wrap items-center gap-2">
                    <span className="text-sm font-medium text-blue-500">Descripción Talla Zapato:</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{item.description_shoesize}</span>
                </div>
            </div>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            <Head title="Tallas de Zapatos" />

            {/* SECCIÓN FIJA: FORMULARIO */}
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Tallas de Zapatos</h2>
                    <p className="text-sm text-muted-foreground">Administra las tallas de calzado para dotación (ej: 38, 40, 42).</p>
                </div>
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                            <div className="flex flex-col gap-4 md:flex-row">
                                ...
                                <div className="grid w-full flex-1 gap-1.5">
                                    <Label htmlFor="desc">Descripción Talla</Label>
                                    <Input
                                        type="number"
                                        id="desc"
                                        placeholder="Ej: 38, 40, 9.5"
                                        value={formData.description_shoesize}
                                        onChange={(e) => setData('description_shoesize', e.target.value)}
                                        disabled={processing}
                                        step="0.1"
                                        min="0"
                                    />
                                    {errors.description_shoesize && <span className="text-xs text-red-500">{errors.description_shoesize}</span>}
                                </div>
