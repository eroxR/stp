...
import { Edit, Eye, EyeOff, GitMerge, PlusCircle, Save, Trash2, X } from 'lucide-react';
...
interface VehicleLine {
    id: number;
    brand_vehicle: string; // Es el código, no el ID
    line_vehicle: string;
    visibility: string;
    company_view: number[] | string[] | null;
    brand?: VehicleBrand; // Relación
}
...
interface Props {
    data: VehicleLine[] | any;
    vehicleBrandsSelect: VehicleBrand[];
}

export default function VehicleLineMaster({ data, companyCode, vehicleBrandsSelect }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/vehicle-lines/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
            ...
            post('/settings/masterTables/vehicle-lines', {
                onSuccess: () => {
                    reset();
    ...
    const handleEdit = (item: VehicleLine) => {
        setEditingId(item.id);
        setData({
            brand_vehicle: item.brand_vehicle,
            line_vehicle: item.line_vehicle,
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            title: '¿Estás seguro?',
            text: 'La línea vehicular se eliminará permanentemente.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/vehicle-lines/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: VehicleLine) => (
        <Card
            key={item.id}
                ...
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        ...
                        <Badge variant="outline" className="border-blue-500 bg-background font-bold text-blue-500">
                            {item.brand?.brand_vehicle || item.brand_vehicle}
                        </Badge>
                    </div>
                </div>
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Nombre de la Línea:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.line_vehicle}</span>
                    </div>
                </div>
            </CardHeader>
    ...
    const renderRow = (item: VehicleLine) => (
        <div
            key={item.id}
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-blue-500">Nombre de la Línea:</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{item.line_vehicle}</span>
                </div>
            </div>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            <Head title="Líneas Vehiculares" />

            {/* SECCIÓN FIJA: FORMULARIO */}
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Líneas Vehiculares</h2>
                    <p className="text-sm text-muted-foreground">Configura las líneas específicas asociadas a las marcas (ej: Mazda 3, Aveo).</p>
                </div>
                            ...
                            <div className="flex flex-col gap-4 md:flex-row">
                                <div className="grid w-full gap-1.5 md:w-1/3">
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(val) => setData('brand_vehicle', val)}
                                        disabled={processing}
                                    >
                                        ...
                                        <SelectContent>
                                            {vehicleBrandsSelect.length > 0 ? (
                                                vehicleBrandsSelect.map((b) => (
                                                    <SelectItem key={b.id} value={b.code_brand_vehicle}>
                                                        {b.brand_vehicle}
                                                    </SelectItem>
                                                ))
                                            ) : (
                                ...
                                <div className="grid w-full flex-1 gap-1.5">
                                    <Label htmlFor="line">Nombre de la Línea</Label>
                                    <Input
                                        id="line"
                                        placeholder="Ej: Picanto, Duster, Logan"
                                        value={formData.line_vehicle}
                                        onChange={(e) => setData('line_vehicle', e.target.value)}
                                        disabled={processing}
                                        maxLength={60}
                                    />
                                    {errors.line_vehicle && <span className="text-xs text-red-500">{errors.line_vehicle}</span>}
                                </div>
                            </div>
