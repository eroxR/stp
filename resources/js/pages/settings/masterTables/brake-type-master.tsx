...
import { Edit, Eye, EyeOff, PlusCircle, RefreshCwOff, Save, Trash2, X } from 'lucide-react'; // Icono de disco
import { useState } from 'react';
import { toast } from 'sonner';
import { CustomTooltip } from '@/components/ui/custom-tooltip';

// Tipos adaptados a la tabla brake_types
interface BrakeType {
    id: number;
...
export default function BrakeTypeMaster({ data }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/brake-types/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
                    toast.success('Tipo de freno actualizado');
                },
                onError: () => toast.error('Error al actualizar'),
            });
        } else {
            post('/settings/masterTables/brake-types', {
                onSuccess: () => {
                    reset();
                    toast.success('Tipo de freno creado');
                },
    ...
    const handleEdit = (item: BrakeType) => {
        setEditingId(item.id);
        setData({
            brake_type_description: item.brake_type_description,
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            title: '¿Estás seguro?',
            text: 'El tipo de freno se eliminará permanentemente.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/brake-types/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: BrakeType) => (
        <Card
            key={item.id}
            ...
            <CardHeader className="pb-2">
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Descripción Tipo de Freno:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.brake_type_description}</span>
                    </div>
                </div>
            </CardHeader>
    ...
    const renderRow = (item: BrakeType) => (
        <div
            key={item.id}
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-blue-100 dark:bg-blue-900/30">
                    ...
                    <RefreshCwOff className="h-4 w-4 text-blue-600 dark:text-blue-400" />
                </div>

                <div className="flex flex-wrap items-center gap-2">
                    <span className="text-sm font-medium text-blue-500">Descripción Tipo de Freno:</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{item.brake_type_description}</span>
                </div>
            </div>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            <Head title="Tipos de Frenos" />

            {/* SECCIÓN FIJA: FORMULARIO */}
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Tipos de Frenos</h2>
                    <p className="text-sm text-muted-foreground">Administra los sistemas de frenos disponibles para los vehículos.</p>
                </div>
                    ...
                    <CardContent className="pt-6">
                        <form onSubmit={handleSubmit} className="flex flex-col items-end gap-4 md:flex-row">
                            <div className="grid w-full flex-1 gap-1.5">
                                <Label htmlFor="desc">Descripción Tipo de Freno</Label>
                                <Input
                                    id="desc"
                                    placeholder="Ej: Aire, Hidráulico, ABS"
                                    value={formData.brake_type_description}
                                    onChange={(e) => setData('brake_type_description', e.target.value)}
                                    disabled={processing}
                                    maxLength={120}
                                />
                                {errors.brake_type_description && <span className="text-xs text-red-500">{errors.brake_type_description}</span>}
                            </div>
