...
import { Edit, Eye, EyeOff, PlusCircle, Save, Trash2, UserCog, X } from 'lucide-react';
...
export default function UserTypeMaster({ data }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/user-types/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
            ...
            post('/settings/masterTables/user-types', {
                onSuccess: () => {
                    reset();
    ...
    const handleEdit = (item: UserType) => {
        ...
        setData({
            description_usertype: item.description_usertype,
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            title: '¿Estás seguro?',
            text: 'El tipo de usuario se eliminará permanentemente.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/user-types/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: UserType) => (
        <Card
            key={item.id}
            ...
            <CardHeader className="pb-2">
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Nombre del Tipo de Usuario:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.description_usertype}</span>
                    </div>
                </div>
            </CardHeader>
    ...
    const renderRow = (item: UserType) => (
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-lime-100 dark:bg-lime-900/30">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{item.description_usertype}</span>
                </div>
            </div>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            ...
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Tipos de Usuario</h2>
                    <p className="text-sm text-muted-foreground">Configura las clasificaciones de usuarios (ej: Empleado, Tercero, Conductor).</p>
                </div>
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                            <div className="flex flex-col gap-4 md:flex-row">
                                ...
                                <div className="grid w-full flex-1 gap-1.5">
                                    <Label htmlFor="desc">Descripción Tipo Usuario</Label>
                                    <Input
                                        id="desc"
                                        placeholder="Ej: Empleado Administrativo"
                                        value={formData.description_usertype}
                                        onChange={(e) => setData('description_usertype', e.target.value)}
                                        disabled={processing}
                                        maxLength={120}
                                    />
                                    {errors.description_usertype && <span className="text-xs text-red-500">{errors.description_usertype}</span>}
                                </div>
