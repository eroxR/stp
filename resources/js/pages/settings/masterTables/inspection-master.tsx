...
import { ClipboardCheck, Edit, Eye, EyeOff, PlusCircle, Save, Trash2, X } from 'lucide-react';
...
interface Inspection {
    ...
    category?: InspectionCategory; // Relación
}

interface FormData {
    name_description: string;
    category_id: string; // String para el select
    visibility: string;
}

interface Props {
    data: Inspection[] | any;
    inspectionCategoriesSelect: InspectionCategory[]; // Para el select
}

export default function InspectionMaster({ data, companyCode, inspectionCategoriesSelect }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/inspections/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
            ...
            post('/settings/masterTables/inspections', {
                onSuccess: () => {
                    reset();
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            title: '¿Estás seguro?',
            text: 'La inspección se eliminará permanentemente.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/inspections/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: Inspection) => (
        <Card
            key={item.id}
                ...
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        ...
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">
                            {item.category?.name_description || item.category_id}
                        </span>
                    </div>
                </div>
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        ...
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.name_description}</span>
                    </div>
                </div>
            </CardHeader>
    ...
    const renderRow = (item: Inspection) => (
        <div
            key={item.id}
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{item.name_description}</span>
                </div>
            </div>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            ...
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Inspecciones</h2>
                    <p className="text-sm text-muted-foreground">Define los ítems específicos a revisar en los vehículos.</p>
                </div>
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                            ...
                            <div className="flex flex-col gap-4 md:flex-row">
                                <div className="grid w-full flex-1 gap-1.5">
                                    <Label htmlFor="name">Nombre Inspección</Label>
                                    <Input
                                        id="name"
                                        placeholder="Ej: Luces delanteras bajas"
                                        value={formData.name_description}
                                        onChange={(e) => setData('name_description', e.target.value)}
                                        disabled={processing}
                                        maxLength={255}
                                    />
                                    {errors.name_description && <span className="text-xs text-red-500">{errors.name_description}</span>}
                                </div>

                                <div className="grid w-full gap-1.5 md:w-1/3">
                                    <Label htmlFor="category">Categoría</Label>
                                    <Select value={formData.category_id} onValueChange={(val) => setData('category_id', val)} disabled={processing}>
                                        <SelectTrigger id="category">
                                            <SelectValue placeholder="Seleccione Categoría" />
                                        </SelectTrigger>
                                        <SelectContent>
                                            {inspectionCategoriesSelect.length > 0 ? (
                                                inspectionCategoriesSelect.map((cat) => (
                                                    <SelectItem key={cat.id} value={cat.id.toString()}>
                                                        {cat.name_description}
                                                    </SelectItem>
                                                ))
                                            ) : (
