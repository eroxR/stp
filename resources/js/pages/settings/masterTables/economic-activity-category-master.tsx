...
import { Edit, Eye, EyeOff, Layers, PlusCircle, Save, Trash2, X } from 'lucide-react';
...
interface Props {
    data: EconomicActivityCategory[] | any;
}

export default function EconomicActivityCategoryMaster({ data }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/economic-activity-categories/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
            ...
            post('/settings/masterTables/economic-activity-categories', {
                onSuccess: () => {
                    reset();
    ...
    const handleEdit = (item: EconomicActivityCategory) => {
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/economic-activity-categories/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: EconomicActivityCategory) => (
        <Card
            key={item.id}
            ...
            <CardHeader className="pb-2">
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">División:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.division}</span>
                    </div>
                </div>
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Grupo:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.groups}</span>
                    </div>
                </div>
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">
                            Descripción Categoria Actividad Económica:
                        </span>
    ...
    const renderRow = (item: EconomicActivityCategory) => (
        <div
            key={item.id}
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <span className="text-sm font-medium text-blue-500">División:</span>
                    <Badge variant="outline" className="bg-background">
                        {item.division}
                    </Badge>

                    <span className="text-sm font-medium text-blue-500">Grupo:</span>
                    <Badge variant="outline" className="bg-background">
                        {item.groups}
                    </Badge>

                    <span className="text-sm font-medium text-blue-500">Descripción Categoria Actividad Económica:</span>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            <Head title="Categorías Económicas" />

            {/* SECCIÓN FIJA: FORMULARIO */}
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Categorías Act. Económicas</h2>
                    <p className="text-sm text-muted-foreground">Configura las divisiones y grupos de las actividades económicas.</p>
                </div>
                            ...
                            <div className="flex flex-col gap-4 md:flex-row">
                                <div className="grid w-full gap-1.5 md:w-32">
                                    ...
                                    <Input
                                        ...
                                        maxLength={255}
                                    />
                                    {errors.division && <span className="text-xs text-red-500">{errors.division}</span>}
                                </div>
                                ...
                                <div className="grid w-full gap-1.5 md:w-32">
                                    ...
                                    <Input
                                        ...
                                        maxLength={255}
                                    />
                                    {errors.groups && <span className="text-xs text-red-500">{errors.groups}</span>}
                                </div>
                                ...
                                <div className="grid w-full flex-1 gap-1.5">
                                    ...
                                    <textarea
                                        id="desc"
                                        placeholder="Ej: Mantenimiento y reparación..."
