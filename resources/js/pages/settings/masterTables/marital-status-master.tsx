...
interface Props {
    data: MaritalStatus[] | any;
}

export default function MaritalStatusMaster({ data }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/marital-statuses/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
            ...
            post('/settings/masterTables/marital-statuses', {
                onSuccess: () => {
                    reset();
    ...
    const handleEdit = (item: MaritalStatus) => {
        ...
        setData({
            description_maritalstatus: item.description_maritalstatus,
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            title: '¿Estás seguro?',
            text: 'El estado civil se eliminará permanentemente.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/marital-statuses/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: MaritalStatus) => (
        <Card
            key={item.id}
            ...
            <CardHeader className="pb-2">
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Descripción Estado Civil:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.description_maritalstatus}</span>
                    </div>
                </div>
            </CardHeader>
    ...
    const renderRow = (item: MaritalStatus) => (
        <div
            key={item.id}
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <span className="text-sm font-medium text-blue-500">Descripción Estado Civil:</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{item.description_maritalstatus}</span>
                </div>
            </div>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            ...
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Estado Civil</h2>
                    <p className="text-sm text-muted-foreground">Administra los estados civiles (ej: Soltero, Casado, Unión Libre).</p>
                </div>
                            ...
                            <div className="flex flex-col gap-4 md:flex-row">
                                ...
                                <div className="grid w-full flex-1 gap-1.5">
                                    ...
                                    <Input
                                        ...
                                        value={formData.description_maritalstatus}
                                        onChange={(e) => setData('description_maritalstatus', e.target.value)}
                                        disabled={processing}
                                        maxLength={120}
                                    />
                                    {errors.description_maritalstatus && (
                                        <span className="text-xs text-red-500">{errors.description_maritalstatus}</span>
                                    )}
                                </div>
