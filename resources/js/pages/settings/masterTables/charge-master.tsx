...
import { Briefcase, Edit, Eye, EyeOff, PlusCircle, Save, Trash2, X } from 'lucide-react';
import { useState } from 'react';
import { toast } from 'sonner';
import { CustomTooltip } from '@/components/ui/custom-tooltip';

// Tipos (asumo que en WorkArea el campo es 'workarea_description', ajústalo si es 'name')
interface WorkArea {
    id: number;
...
interface Props {
    data: Charge[] | any;
    workAreasSelect: WorkArea[];
}

export default function ChargeMaster({ data, companyCode, workAreasSelect }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/charges/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
            ...
            post('/settings/masterTables/charges', {
                onSuccess: () => {
                    reset();
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            title: '¿Estás seguro?',
            text: 'El cargo se eliminará permanentemente.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/charges/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
            ...
            <CardHeader className="pb-2">
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Area Laboral:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">
                            {item.work_area?.workarea_description || 'Sin área asignada'}
                        </span>
                    </div>
                </div>
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Codigo del Cargo:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.code_charge}</span>
                    </div>
                </div>
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Descipción del Cargo:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.description_charge}</span>
                    </div>
                </div>
            </CardHeader>
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-blue-500">Nombre del Permiso:</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{item.description_charge}</span>
                    <span className="text-sm font-medium text-blue-500">Area Laboral:</span>
                    <Badge variant="outline" className="bg-background">
                        {item.work_area?.workarea_description || 'Sin área asignada'}
                    </Badge>
                </div>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            <Head title="Cargos Empresariales" />

            {/* SECCIÓN FIJA: FORMULARIO */}
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Cargos</h2>
                    <p className="text-sm text-muted-foreground">Administra los cargos laborales y su asociación con áreas.</p>
                </div>
                ...
                <Card className="border-l-4 border-l-blue-600 bg-muted/20 shadow-sm dark:bg-muted/10">
                    <CardContent className="pt-6">
                        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                            ...
                            <div className="flex flex-col gap-4 md:flex-row">
                                <div className="grid w-full gap-1.5 md:w-32">
                                    ...
                                    <Input
                                        id="code"
                                        placeholder="Ej: A01"
                                        value={formData.code_charge}
                                        onChange={(e) => setData('code_charge', e.target.value)}
                                        disabled={processing}
                                        maxLength={5}
                                    />
                                    {errors.code_charge && <span className="text-xs text-red-500">{errors.code_charge}</span>}
                                </div>

                                <div className="grid w-full flex-1 gap-1.5">
                                    <Label htmlFor="desc">Nombre del Cargo</Label>
                                    <Input
                                        id="desc"
                                        placeholder="Ej: Analista de Sistemas"
                                        value={formData.description_charge}
                                        onChange={(e) => setData('description_charge', e.target.value)}
                                        disabled={processing}
                                        maxLength={120}
                                    />
                                    {errors.description_charge && <span className="text-xs text-red-500">{errors.description_charge}</span>}
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-4 md:flex-row">
                                <div className="grid w-full flex-1 gap-1.5">
                                    <Label htmlFor="area">Área Asociada</Label>
                                    <Select value={formData.area} onValueChange={(val) => setData('area', val)} disabled={processing}>
                                        <SelectTrigger id="area">
                                            <SelectValue placeholder="Seleccione un área de trabajo" />
                                        </SelectTrigger>
                                        <SelectContent>
                                            {workAreasSelect.length > 0 ? (
                                                workAreasSelect.map((area) => (
                                                    <SelectItem key={area.id} value={area.id.toString()}>
                                                        {area.workarea_description}
                                                    </SelectItem>
                                                ))
                                            ) : (
