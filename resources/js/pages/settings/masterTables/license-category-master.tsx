...
interface Props {
    data: LicenseCategory[] | any;
}

export default function LicenseCategoryMaster({ data }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/license-categories/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
            ...
            post('/settings/masterTables/license-categories', {
                onSuccess: () => {
                    reset();
    ...
    const handleEdit = (item: LicenseCategory) => {
        setEditingId(item.id);
        setData({
            code_licensecategory: item.code_licensecategory,
            description_licensecategory: item.description_licensecategory,
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            title: '¿Estás seguro?',
            text: 'La categoría de licencia se eliminará permanentemente.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/license-categories/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: LicenseCategory) => (
        <Card
            key={item.id}
            ...
            <CardHeader className="pb-2">
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Codigo Categoría Licencia:</span>
                        <Badge variant="outline" className="border-blue-500 bg-background font-bold text-blue-500">
                            {item.code_licensecategory}
                        </Badge>
                    </div>
                </div>
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Descripción Categoría Licencia:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.description_licensecategory}</span>
                    </div>
                </div>
            </CardHeader>
    ...
    const renderRow = (item: LicenseCategory) => (
        <div
            key={item.id}
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <span className="text-sm font-medium text-blue-500">Código Categoria de Licencia:</span>
                    <Badge variant="outline" className="bg-background">
                        {item.code_licensecategory}
                    </Badge>
                    <span className="text-sm font-medium text-blue-500">Descripción Categoría Licencia:</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{item.description_licensecategory}</span>
                </div>
            </div>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            ...
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Licencias</h2>
                    <p className="text-sm text-muted-foreground">Configura las categorías de licencias de conducción (ej: A2, B1, C3).</p>
                </div>
                            ...
                            <div className="flex flex-col gap-4 md:flex-row">
                                ...
                                <div className="grid w-full gap-1.5 md:w-32">
                                    ...
                                    <Input
                                        id="code"
                                        placeholder="Ej: C1"
                                        value={formData.code_licensecategory}
                                        onChange={(e) => setData('code_licensecategory', e.target.value.toUpperCase())}
                                        disabled={processing}
                                        maxLength={4}
                                    />
                                    {errors.code_licensecategory && <span className="text-xs text-red-500">{errors.code_licensecategory}</span>}
                                </div>
                                ...
                                <div className="grid w-full flex-1 gap-1.5">
                                    ...
                                    <textarea
                                        ...
                                        value={formData.description_licensecategory}
                                        onChange={(e) => setData('description_licensecategory', e.target.value)}
                                        disabled={processing}
                                        maxLength={255}
