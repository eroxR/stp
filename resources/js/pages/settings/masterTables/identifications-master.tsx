...
import { Edit, Eye, EyeOff, IdCard, PlusCircle, Save, Trash2, X } from 'lucide-react';
...
// Props aceptan Array O Paginación (gracias al 'any' controlado en MainMaster, esto funcionará)
// Si quieres ser estricto aquí, importarías PaginatedData de master-view-layout
interface Props {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    data: Identification[] | any;
}

export default function IdentificationsMaster({ data }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (editingId) {
            put(`/settings/masterTables/identifications/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
            ...
            post('/settings/masterTables/identifications', {
                onSuccess: () => {
                    reset();
    ...
    const handleEdit = (item: Identification) => {
        setEditingId(item.id);
        setData({
            description_identification: item.description_identification,
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            title: '¿Estás seguro?',
            text: 'No podrás revertir esta acción. El registro se eliminará permanentemente.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/identifications/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: Identification) => (
        <Card
            key={item.id}
            ...
            <CardHeader className="pb-2">
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Descripción Identificación:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.description_identification}</span>
                    </div>
                </div>
            </CardHeader>
    ...
    const renderRow = (item: Identification) => (
        <div
            key={item.id}
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <span className="text-sm font-medium text-blue-500">Descripción Identificación:</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{item.description_identification}</span>
                </div>
            </div>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            <Head title="Identificaciones" />

            {/* SECCIÓN FIJA: FORMULARIO */}
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Tipo de Identificaciones</h2>
                    <p className="text-sm text-muted-foreground">Administra los documentos de identidad válidos en el sistema.</p>
                </div>
                    ...
                    <CardContent className="pt-6">
                        <form onSubmit={handleSubmit} className="flex flex-col items-end gap-4 md:flex-row">
                            <div className="grid w-full flex-1 gap-1.5">
                                <Label htmlFor="desc">Descripción Identificación</Label>
                                <Input
                                    id="desc"
                                    placeholder="Ej: Cédula de Ciudadanía"
                                    value={formData.description_identification}
                                    onChange={(e) => setData('description_identification', e.target.value)}
                                    disabled={processing}
                                />
                                {errors.description_identification && (
                                    <span className="text-xs text-red-500">{errors.description_identification}</span>
                                )}
                            </div>
