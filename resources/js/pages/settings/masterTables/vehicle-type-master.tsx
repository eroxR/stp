...
import { CarFront, Edit, Eye, EyeOff, PlusCircle, Save, Trash2, X } from 'lucide-react';
...
export default function VehicleTypeMaster({ data }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/vehicle-types/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
            ...
            post('/settings/masterTables/vehicle-types', {
                onSuccess: () => {
                    reset();
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            title: '¿Estás seguro?',
            text: 'El tipo de vehículo se eliminará permanentemente.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/vehicle-types/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: VehicleType) => (
        <Card
            key={item.id}
            ...
            <CardHeader className="pb-2">
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Nombre del Tipo de Vehículo:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.vehicle_type_name}</span>
                    </div>
                </div>
            </CardHeader>
    ...
    const renderRow = (item: VehicleType) => (
        <div
            key={item.id}
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800">
                    <CarFront className="h-4 w-4 text-slate-600 dark:text-slate-400" />
                </div>

                <div className="flex flex-wrap items-center gap-2">
                    <span className="text-sm font-medium text-blue-500">Nombre del Tipo de Vehículo:</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{item.vehicle_type_name}</span>
                </div>
            </div>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            <Head title="Tipos de Vehículos" />

            {/* SECCIÓN FIJA: FORMULARIO */}
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Tipos de Vehículos</h2>
                    <p className="text-sm text-muted-foreground">Configura las clases de vehículos permitidos (ej: Bus, Camión, Van).</p>
                </div>
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                            <div className="flex flex-col gap-4 md:flex-row">
                                ...
                                <div className="grid w-full flex-1 gap-1.5">
                                    <Label htmlFor="name">Nombre del Tipo de Vehículo</Label>
                                    <Input
                                        id="name"
                                        placeholder="Ej: Microbus, Camioneta, Buseton"
                                        value={formData.vehicle_type_name}
                                        onChange={(e) => setData('vehicle_type_name', e.target.value)}
                                        disabled={processing}
                                        maxLength={120}
                                    />
                                    {errors.vehicle_type_name && <span className="text-xs text-red-500">{errors.vehicle_type_name}</span>}
                                </div>
