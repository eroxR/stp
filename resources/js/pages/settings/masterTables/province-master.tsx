...
import { Edit, Eye, EyeOff, Map, PlusCircle, Save, Trash2, X } from 'lucide-react';
...
interface Province {
    ...
    partner_country: number; // ID del país
    visibility: string;
    company_view: number[] | string[] | null;
    country?: Country; // Relación cargada
}

interface FormData {
    department_name: string;
    partner_country: string; // Manejamos como string para el Select
    visibility: string;
}

interface Props {
    data: Province[] | any;
    countriesSelect: Country[]; // Lista para el select
}

export default function ProvinceMaster({ data, companyCode, countriesSelect }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/provinces/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
            ...
            post('/settings/masterTables/provinces', {
                onSuccess: () => {
                    reset();
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/provinces/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: Province) => (
        <Card
            key={item.id}
        ...
        >
            <CardHeader className="pb-2">
                ...
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Nombre del Departamento/Provincia:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.department_name}</span>
                    </div>
                </div>
            </CardHeader>
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{item.department_name}</span>
                </div>
            </div>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            ...
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Departamentos</h2>
                    <p className="text-sm text-muted-foreground">Administra los departamentos o provincias asociados a un país.</p>
                </div>
                            ...
                            <div className="flex flex-col gap-4 md:flex-row">
                                <div className="grid w-full gap-1.5 md:w-1/3">
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(val) => setData('partner_country', val)}
                                        disabled={processing}
                                    >
                                ...
                                <div className="grid w-full flex-1 gap-1.5">
                                    <Label htmlFor="name">Nombre Departamento/Provincia</Label>
                                    <div className="relative">
                                        <Map className="absolute top-2.5 left-2.5 h-4 w-4 text-muted-foreground" />
                                        <Input
                                            id="name"
                                            className="pl-9"
