...
import { Edit, Eye, EyeOff, Files, PlusCircle, Save, Trash2, X } from 'lucide-react';
...
interface Props {
    data: ResourceDocument[] | any;
}

export default function ResourceDocumentMaster({ data }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/resource-documents/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
            ...
            post('/settings/masterTables/resource-documents', {
                onSuccess: () => {
                    reset();
    ...
    const handleEdit = (item: ResourceDocument) => {
        ...
        setData({
            name_document: item.name_document,
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/resource-documents/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: ResourceDocument) => (
        <Card
            key={item.id}
            ...
            <CardHeader className="pb-2">
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Nombre Documento:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.name_document}</span>
                    </div>
                </div>
            </CardHeader>
    ...
    const renderRow = (item: ResourceDocument) => (
        <div
            key={item.id}
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <span className="text-sm font-medium text-blue-500">Nombre del Documento:</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{item.name_document}</span>
                </div>
            </div>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            <Head title="Recurso Documental" />

            {/* SECCIÓN FIJA: FORMULARIO */}
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Recurso Documental</h2>
                    <p className="text-sm text-muted-foreground">
                        Define los documentos requeridos para el seguimiento de recursos (ej: Licencia, SOAT).
                    </p>
                </div>
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                            <div className="flex flex-col gap-4 md:flex-row">
                                ...
                                <div className="grid w-full flex-1 gap-1.5">
                                    ...
                                    <Input
                                        id="name"
                                        placeholder="Ej: Licencia de Conducción, Cédula"
                                        value={formData.name_document}
                                        onChange={(e) => setData('name_document', e.target.value)}
                                        disabled={processing}
                                        maxLength={255}
                                    />
                                    {errors.name_document && <span className="text-xs text-red-500">{errors.name_document}</span>}
                                </div>
