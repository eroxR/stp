...
interface Props {
    data: InspectionCategory[] | any;
}

export default function InspectionCategoryMaster({ data }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/inspection-categories/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
            ...
            post('/settings/masterTables/inspection-categories', {
                onSuccess: () => {
                    reset();
    ...
    const handleEdit = (item: InspectionCategory) => {
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/inspection-categories/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: InspectionCategory) => (
        <Card
            key={item.id}
    ...
    const renderRow = (item: InspectionCategory) => (
        <div
            key={item.id}
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            <Head title="Categorías de Inspección" />

            {/* SECCIÓN FIJA: FORMULARIO */}
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Categorías de Inspección</h2>
                    <p className="text-sm text-muted-foreground">Configura los tipos o categorías para las inspecciones de vehículos.</p>
                </div>
