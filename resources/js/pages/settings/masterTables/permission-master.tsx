...
import { Edit, Eye, EyeOff, Key, PlusCircle, Save, Trash2, X } from 'lucide-react';
...
export default function PermissionMaster({ data }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/permissions/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
            ...
            post('/settings/masterTables/permissions', {
                onSuccess: () => {
                    reset();
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            title: '¿Estás seguro?',
            text: 'El permiso se eliminará permanentemente. Esto podría afectar a los roles asignados.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/permissions/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: Permission) => (
        <Card
            key={item.id}
            ...
            <CardHeader className="pb-2">
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Nombre del Guard:</span>
                ...
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Nombre del Permiso:</span>
    ...
    const renderRow = (item: Permission) => (
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-yellow-100 dark:bg-yellow-900/30">
                    <Key className="h-4 w-4 text-yellow-600 dark:text-yellow-400" />
                </div>

                <div className="flex flex-wrap items-center gap-2">
                    <span className="text-sm font-medium text-blue-500">Nombre del Guard:</span>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            ...
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Permisos</h2>
                    <p className="text-sm text-muted-foreground">Administra los permisos técnicos del sistema (Spatie).</p>
                </div>
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                            <div className="flex flex-col gap-4 md:flex-row">
                                ...
                                <div className="grid w-full flex-1 gap-1.5">
                                    ...
                                    <Input
                                        id="name"
                                        placeholder="Ej: crear_usuarios, editar_vehiculos"
                                        value={formData.name}
                                ...
                                <div className="grid w-full gap-1.5 md:w-48">
                                    ...
                                    <Input
                                        ...
                                        maxLength={255}
                                    />
                                    {errors.guard_name && <span className="text-xs text-red-500">{errors.guard_name}</span>}
                                </div>
