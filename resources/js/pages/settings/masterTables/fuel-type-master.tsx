...
export default function FuelTypeMaster({ data }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/fuel-types/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
                    toast.success('Tipo de combustible actualizado');
                },
                onError: () => toast.error('Error al actualizar'),
            });
        } else {
            post('/settings/masterTables/fuel-types', {
                onSuccess: () => {
                    reset();
                    toast.success('Tipo de combustible creado');
                },
    ...
    const handleEdit = (item: FuelType) => {
        ...
        setData({
            fuel_types_description: item.fuel_types_description,
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            title: '¿Estás seguro?',
            text: 'El tipo de combustible se eliminará permanentemente.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/fuel-types/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: FuelType) => (
        <Card
            key={item.id}
            ...
            <CardHeader className="pb-2">
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Descripción Combustible:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.fuel_types_description}</span>
                    </div>
                </div>
            </CardHeader>
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                ...
                <div className="flex min-w-0 flex-col">
                    <span className="text-sm font-medium text-blue-500">Descripción Combustible:</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{item.fuel_types_description}</span>
                </div>
            </div>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            ...
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Combustibles</h2>
                    <p className="text-sm text-muted-foreground">Configura los tipos de combustible (ej: Gasolina, Diesel, Eléctrico).</p>
                </div>
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                            <div className="flex flex-col gap-4 md:flex-row">
                                ...
                                <div className="grid w-full flex-1 gap-1.5">
                                    <Label htmlFor="desc">Descripción Combustible</Label>
                                    <Input
                                        id="desc"
                                        placeholder="Ej: Gasolina Corriente, Diesel"
                                        value={formData.fuel_types_description}
                                        onChange={(e) => setData('fuel_types_description', e.target.value)}
                                        disabled={processing}
                                        maxLength={120}
                                    />
                                    {errors.fuel_types_description && <span className="text-xs text-red-500">{errors.fuel_types_description}</span>}
                                </div>
