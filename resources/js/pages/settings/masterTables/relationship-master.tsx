...
interface Props {
    data: Relationship[] | any;
}

export default function RelationshipMaster({ data }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/relationships/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
            ...
            post('/settings/masterTables/relationships', {
                onSuccess: () => {
                    reset();
    ...
    const handleEdit = (item: Relationship) => {
        ...
        setData({
            description_relationship: item.description_relationship,
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            title: '¿Estás seguro?',
            text: 'El parentesco se eliminará permanentemente.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/relationships/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: Relationship) => (
        <Card
            key={item.id}
            ...
            <CardHeader className="pb-2">
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Descripción Parentesco:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.description_relationship}</span>
                    </div>
                </div>
            </CardHeader>
    ...
    const renderRow = (item: Relationship) => (
        <div
            key={item.id}
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <span className="text-sm font-medium text-blue-500">Descripción Parentesco:</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{item.description_relationship}</span>
                </div>
            </div>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            <Head title="Parentescos" />

            {/* SECCIÓN FIJA: FORMULARIO */}
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Parentescos</h2>
                    <p className="text-sm text-muted-foreground">Administra los tipos de relación familiar (ej: Padre, Madre, Hijo).</p>
                </div>
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                            <div className="flex flex-col gap-4 md:flex-row">
                                ...
                                <div className="grid w-full flex-1 gap-1.5">
                                    <Label htmlFor="desc">Descripción Parentesco</Label>
                                    <Input
                                        id="desc"
                                        placeholder="Ej: Padre, Hermano, Cónyuge"
                                        value={formData.description_relationship}
                                        onChange={(e) => setData('description_relationship', e.target.value)}
                                        disabled={processing}
                                        maxLength={45}
                                    />
                                    {errors.description_relationship && (
                                        <span className="text-xs text-red-500">{errors.description_relationship}</span>
                                    )}
                                </div>
