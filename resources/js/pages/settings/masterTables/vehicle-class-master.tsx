...
interface Props {
    data: VehicleClass[] | any;
}

export default function VehicleClassMaster({ data }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/vehicle-classes/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
                    toast.success('Clase de vehículo actualizada');
                },
                onError: () => toast.error('Error al actualizar'),
            });
        } else {
            post('/settings/masterTables/vehicle-classes', {
                onSuccess: () => {
                    reset();
                    toast.success('Clase de vehículo creada');
                },
    ...
    const handleEdit = (item: VehicleClass) => {
        setEditingId(item.id);
        setData({
            vehicle_class_description: item.vehicle_class_description,
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            title: '¿Estás seguro?',
            text: 'La clase de vehículo se eliminará permanentemente.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/vehicle-classes/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: VehicleClass) => (
        <Card
            key={item.id}
            ...
            <CardHeader className="pb-2">
                <div className="w-full">
                    <div className="flex flex-wrap items-start gap-2">
                        <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Descripción de la Clase:</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.vehicle_class_description}</span>
                    </div>
                </div>
            </CardHeader>
    ...
    const renderRow = (item: VehicleClass) => (
        <div
            key={item.id}
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <span className="text-sm font-medium text-blue-500">Nombre de la Clase:</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{item.vehicle_class_description}</span>
                </div>
            </div>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            <Head title="Clases de Vehículos" />

            {/* SECCIÓN FIJA: FORMULARIO */}
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Clases de Vehículos</h2>
                    <p className="text-sm text-muted-foreground">Configura las clases de servicio del vehículo (ej: Público, Particular).</p>
                </div>
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                            <div className="flex flex-col gap-4 md:flex-row">
                                ...
                                <div className="grid w-full flex-1 gap-1.5">
                                    <Label htmlFor="desc">Descripción Clase</Label>
                                    <Input
                                        id="desc"
                                        placeholder="Ej: Servicio Público, Particular"
                                        value={formData.vehicle_class_description}
                                        onChange={(e) => setData('vehicle_class_description', e.target.value)}
                                        disabled={processing}
                                        maxLength={120}
                                    />
                                    {errors.vehicle_class_description && (
                                        <span className="text-xs text-red-500">{errors.vehicle_class_description}</span>
                                    )}
                                </div>
