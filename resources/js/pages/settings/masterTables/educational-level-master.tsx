...
import { BookOpenText, Edit, Eye, EyeOff, PlusCircle, Save, Trash2, X } from 'lucide-react';
import { useState } from 'react';
import { toast } from 'sonner';
import { CustomTooltip } from '@/components/ui/custom-tooltip';

// Tipos adaptados a la tabla educational_levels
interface EducationalLevel {
    id: number;
    description_leveleducation: string; // <--- Campo cambiado
    visibility: string;
...
interface Props {
    data: EducationalLevel[] | any;
}

export default function EducationalLevelMaster({ data }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            put(`/settings/masterTables/educational-levels/${editingId}`, {
                onSuccess: () => {
                    handleCancel();
            ...
            post('/settings/masterTables/educational-levels', {
                onSuccess: () => {
                    reset();
    ...
    const handleEdit = (item: EducationalLevel) => {
        setEditingId(item.id);
        setData({
            description_leveleducation: item.description_leveleducation,
    ...
    const handleDelete = (id: number) => {
        MySwal.fire({
            title: '¿Estás seguro?',
            text: 'El nivel educativo se eliminará permanentemente.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                destroy(`/settings/masterTables/educational-levels/${id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                        MySwal.fire({
                            title: '¡Eliminado!',
    ...
    const renderCard = (item: EducationalLevel) => (
        <Card
            key={item.id}
        ...
        >
            <CardHeader className="pb-2">
                <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Descripción Nivel Educativo:</span>
                <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.description_leveleducation}</span>
            </CardHeader>
    ...
    const renderRow = (item: EducationalLevel) => (
        <div
            key={item.id}
        ...
        >
            ...
            <div className="flex items-center gap-4 overflow-hidden">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <span className="text-sm font-medium text-blue-500">Descripción Nivel Educativo:</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{item.description_leveleducation}</span>
                </div>
            </div>
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            ...
            <div className="flex-none space-y-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Maestra de Niveles Educativos</h2>
                    <p className="text-sm text-muted-foreground">Administra los niveles de escolaridad del sistema.</p>
                </div>
                    ...
                    <CardContent className="pt-6">
                        <form onSubmit={handleSubmit} className="flex flex-col items-end gap-4 md:flex-row">
                            <div className="grid w-full flex-1 gap-1.5">
                                <Label htmlFor="desc">Descripción Nivel Educativo</Label>
                                <Input
                                    id="desc"
                                    placeholder="Ej: Secundaria, Universitario, Posgrado"
                                    value={formData.description_leveleducation}
                                    onChange={(e) => setData('description_leveleducation', e.target.value)}
                                    disabled={processing}
                                />
                                {errors.description_leveleducation && (
                                    <span className="text-xs text-red-500">{errors.description_leveleducation}</span>
                                )}
                            </div>
